...
async fn list_sessions(_auth: AuthUser) -> Json<serde_json::Value> {
    Json(serde_json::json!([
...
async fn delete_session(_auth: AuthUser, Path(_id): Path<String>) -> Json<StatusResp> {
    Json(StatusResp { status: "revoked" })
}

async fn delete_all_sessions(_auth: AuthUser) -> Json<StatusResp> {
    Json(StatusResp { status: "all_revoked" })
}

// ── Risk ──────────────────────────────────────────────────────────────────────

async fn risk_users(_auth: AuthUser) -> Json<serde_json::Value> {
    Json(serde_json::json!([
...
async fn risk_feed(_auth: AuthUser) -> Json<serde_json::Value> {
    Json(serde_json::json!([
...
async fn risk_scan(_auth: AuthUser) -> Json<serde_json::Value> {
    Json(serde_json::json!({ "status": "scan_complete", "updated": 3, "high": 1, "medium": 1, "low": 1 }))
}

// ── Users & Roles ─────────────────────────────────────────────────────────────

async fn list_users(_auth: AuthUser) -> Json<serde_json::Value> {
    Json(serde_json::json!([
...
async fn patch_user_role(
    ...
    Json(_body): Json<RolePatch>,
) -> Json<StatusResp> {
    Json(StatusResp { status: "updated" })
}

// ── Passkeys ──────────────────────────────────────────────────────────────────

async fn list_passkeys(_auth: AuthUser) -> Json<serde_json::Value> {
    Json(serde_json::json!([
...
async fn delete_passkey(_auth: AuthUser, Path(_id): Path<String>) -> Json<StatusResp> {
    Json(StatusResp { status: "revoked" })
}

// ── Policies ──────────────────────────────────────────────────────────────────

async fn list_policies(_auth: AuthUser) -> Json<serde_json::Value> {
    Json(serde_json::json!([
...
async fn get_org(_auth: AuthUser) -> Json<serde_json::Value> {
    Json(serde_json::json!({
