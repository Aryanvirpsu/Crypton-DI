...
export default function Recovery({ go, toast }) {
  const [request, setRequest]   = useState(null); // backend recovery_request object
  ...
  const startRecovery = async () => {
    try {
      const req = await api.post("/recovery/start", { method: "trusted_device" });
      setRequest(req);
  ...
  const approveRecovery = async () => {
    if (!request) return;
    try {
      const req = await api.post("/recovery/approve", { request_id: request.id });
      setRequest(req);
  ...
  const completeRecovery = async () => {
    if (!request) return;
    try {
      const req = await api.post("/recovery/complete", { request_id: request.id });
      setRequest(req);
      clearInterval(intRef.current);
