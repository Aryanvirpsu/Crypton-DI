...
import { TOKEN_KEY, getToken, setToken, clearToken, parseJwt, PROTECTED_PAGES, _authRef } from './auth';
import { api } from './api';
...
import { useToasts, useReveal } from './hooks';
...
// ── CONFIG ─────────────────────────────────────────────────────
//
//   API_BASE controls where all fetch() calls are sent.
//
//   Empty string (default):
//     Requests are relative to the current origin.
//     CRA dev proxy (package.json "proxy": "http://localhost:8090") forwards
//     them to the gateway on :8090, which proxies to identity on :8080.
//     Works for both localhost:3000 AND Cloudflare tunnel (the proxy is
//     server-side in Node, so it fires regardless of how the browser arrived).
//
//   REACT_APP_API_BASE set (e.g. "https://your-tunnel.trycloudflare.com"):
//     Requests become absolute URLs, bypassing the CRA proxy entirely.
//     Use this when the gateway is publicly reachable on its own domain,
//     or when you need to point the frontend at a remote/staging backend.
//     The gateway must have CORS configured to allow the frontend origin.
//
      ...
      <div ref={navRef} className={`landing-nav${scrolled ? " scrolled" : ""}`} style={{ opacity: 0, zIndex: 600 }}>
        ...
        <div className="nav-desktop-btns" style={{ display: "flex", gap: 10 }}>
          {isLoggedIn
            ...
            : <>
                <button onClick={() => go("login")} style={{ fontFamily: "var(--mono)", fontSize: 10, letterSpacing: ".1em", textTransform: "uppercase", color: "var(--muted)", background: "none", padding: "10px 16px", border: "1px solid var(--line2)", cursor: "pointer", transition: "color .2s" }}
                  onMouseEnter={e => e.currentTarget.style.color = "var(--paper)"} onMouseLeave={e => e.currentTarget.style.color = "var(--muted)"}>Sign In</button>
          ...
          <div className="rv manifesto-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "start" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                ...
                <BtnO onClick={() => go("login")}>Sign In →</BtnO>
              </div>
            </div>
...
if (PROTECTED_PAGES.has(initialPage) && !getToken()) {
  ...
  // Keep module-level ref in sync so 401 handler and doLogin can update auth state
  ...
  return (
    <AuthContext.Provider value={authCtx}>
      ...
      {authReady && page === "dashboard"  && <Dashboard go={go} toast={toast} />}
      {authReady && page === "devices"    && <Devices go={go} toast={toast} />}
      {authReady && page === "recovery"   && <Recovery go={go} toast={toast} />}
      {authReady && page === "admin"      && <Admin go={go} toast={toast} />}
      {authReady && page === "auditlogs"  && <AuditLogs go={go} toast={toast} />}
      {authReady && page === "actions"    && <ProtectedActions go={go} toast={toast} />}
      {authReady && page === "risk"       && <RiskIntel go={go} toast={toast} />}
      {authReady && page === "sessions"   && <Sessions go={go} toast={toast} />}
      {authReady && page === "rbac"       && <RBAC go={go} toast={toast} />}
      {authReady && page === "policy"     && <PolicyEngine go={go} toast={toast} />}
      {authReady && page === "orgsettings"&& <OrgSettings go={go} toast={toast} />}
      {simState !== 'closed' && <AttackSim animState={simState} onClose={closeSim} />}
    </AuthContext.Provider>
  );
}

export { api, getToken, setToken, clearToken, parseJwt, b64url, fromB64url, BtnF, BtnO, AppShell, PAGE_LABELS };
